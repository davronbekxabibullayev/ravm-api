...
public class UpdateWaybillMechanicConclusionCommandValidator
    : AbstractValidator<UpdateWaybillMechanicConclusionCommand>
{
    public UpdateWaybillMechanicConclusionCommandValidator()
    {
        ...
        RuleFor(x => x.ReturnedDriverId)
            .NotEqual(Guid.Empty)
        .NotEmpty()
        .When(x => x.MechanicConclusionType == Domain.Enums.MechanicConclusionType.acceptance);
    }
}
